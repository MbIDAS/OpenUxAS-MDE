...
class UxasXMLRenderer:
    ...
    def resolve_references(self, obj_value, env):
        if isinstance(obj_value, dict):
            if "category" in obj_value:
                category = env.lookup(obj_value["category"])
                if category is None:
                    raise RenderError("No such collection named "+obj_value["category"])
                item = None
                obj_chain = ""
                if "name" in obj_value and obj_value["name"] != "":
                    for c in category:
                ...
                for param in obj_value["param_list"]:
                    if param not in item:
                        raise RenderError("No param named "+param+" in "+obj_chain+
                                      " in "+obj_value["category"])
                    obj_chain = obj_chain + "." + param
                    item = item[param]
                obj_value = item
        return obj_value
    ...
    def get_value(self, obj, value, env, foreach_map):
        if value["value_type"] == "string":
            ...
            obj_value = self.resolve_references(obj[value["name"]], env)
            return obj_value
        elif value["value_type"] == "current":
            return self.resolve_references(obj, env)
        ...
        if type(obj) is dict:
            if "variable_definitions" in obj:
                ...
                for variable_definition in obj["variable_definitions"]:
                    referenced = self.resolve_references(variable_definition["reference"], env)
                    new_ve = []
